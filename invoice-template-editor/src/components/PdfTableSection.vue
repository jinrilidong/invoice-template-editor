<template>
  <div class="w-full mb-0">
    ...
    <div v-for="(table, tableIndex) in tablesData" :key="table.id" :id="`config-table-${tableIndex}-section`" class="mb-8">
      <!-- Divider between sections (except for the first one) - 已移除，由Configuration Panel统一管理 -->
      
      <!-- Section Header with Title and Actions -->
      <div class="flex items-center justify-between mb-4" :style="{ marginTop: tableIndex > 0 ? '20px' : '20px' }">
      ...
      <div v-if="isTableExpanded(tableIndex)" class="space-y-4">
       ...
       <div :id="`config-table-${tableIndex}-section-title`">
         <UnifiedLInput
           ...
           @update:model-value="(value: string) => updateTableField(tableIndex, 'sectionTitle', value)"
           label="Section Title"
         />
       </div>
       ...
       <div :id="`config-table-${tableIndex}-subsection-title`">
         <UnifiedLInput
           ...
           @update:model-value="(value: string) => updateTableField(tableIndex, 'subsectionTitle', value)"
       ...
       <div class="mb-5">
             ...
             <div class="space-y-3">
               ...
               <div :id="`config-table-${tableIndex}-column-header-${column.id}`">
                 <UnifiedLInput
                   ...
                   @update:model-value="(value: string) => updateColumnField(tableIndex, index, 'name', value)"
                   label="Column Name"
                 />
               </div>
               <!-- Column Width Input -->
               <div :id="`config-table-${tableIndex}-column-width-${column.id}`">
                 <UnifiedLInput
                   ...
                   @update:model-value="(value: string) => updateColumnField(tableIndex, index, 'width', Number(value) || 100)"
                   label="Column Width (%)"
                   type="number"
                 />
               </div>
               ...
               <div class="flex items-center gap-3">
                 <Toggle 
                   ...
                   @update:model-value="(value) => updateColumnField(tableIndex, index, 'alignment', value ? 'right' : 'left')"
                   size="sm"
                 />
        ...
        <!-- Column Value -->
         <div class="mt-5" :id="`config-table-${tableIndex}-column-value-section`">
           <h4 class="text-sm font-medium text-gray-600" style="margin-bottom: 20px;">Column Value</h4>
           <div class="space-y-1">
                 ...
                 <div v-for="(column, colIndex) in table.columns" :key="column.id">
                   <div :id="`config-table-${tableIndex}-column-value-${column.id}`">
                     <UnifiedLInput
                       ...
                       @update:model-value="(value: string) => updateColumnTemplate(tableIndex, column.id, value)"
        ...
        <div class="mt-5 mb-5">
          ...
          <div style="margin-bottom: 20px;">
            <UnifiedLInput
              ...
              @update:model-value="(value: string) => updateTableField(tableIndex, 'rowsNumber', value)"
              label="Rows Number"
              type="number"
            />
          </div>
...
import type { TableData, TableColumn, TableRow, BaseSectionProps, BaseSectionEmits } from '../types/section'
...
const updateTableField = (tableIndex: number, field: keyof TableData, value: any) => {
  ...
  if (newTables[tableIndex]) {
    // 确保 rowsNumber 是数字类型
    const processedValue = field === 'rowsNumber' ? Number(value) || 2 : value
...
// 更新列字段
const updateColumnField = (tableIndex: number, columnIndex: number, field: keyof TableColumn, value: any) => {
  const newTables = [...tablesData.value]
...
const addColumn = (tableIndex: number) => {
  ...
  if (table && table.columns) {
    ...
    const newRows = table.rows ? table.rows.map(row => ({
      ...row,
