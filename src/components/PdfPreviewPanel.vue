  ...
  <div class="h-full bg-white rounded-xl shadow-lg border border-[#d3ddde] flex flex-col">
    ...
    <div ref="containerRef" class="flex-1 overflow-y-auto overflow-x-hidden bg-[#f8f9fa]">
      <div v-if="!props.exportMode" class="w-full flex justify-center" style="padding-top: 20px;">
        <!-- Single PDF Page -->
        <div class="pdf-pages-container" :style="{ transform: `scale(${zoomLevel})`, transformOrigin: 'top center' }">
              <!-- PDF Page -->
              <div 
            ...
            >
              <div class="flex flex-col gap-px items-start justify-center">
                <h1 class="font-semibold text-black leading-6" style="font-size: 20px;">{{ props.templateData.header?.title || 'Title' }}</h1>
                <p v-if="props.templateData.header?.description" class="font-normal leading-2.25" style="font-size: 7px;">
              ...
              <div class="logo-container w-60 overflow-hidden relative flex items-center justify-end"
                   :style="{ height: (props.templateData.header?.logoSize === 'large' ? 72 : 48) + 'px' }">
                <img 
            ...
            >
              ...
              >
                <p v-if="infoSection.sectionTitle" class="info-section-title">{{ infoSection.sectionTitle }}</p>
            ...
            >
              <div 
                ...
                :key="table.id"
                v-show="console.log('Table data check:', table, 'Index:', tableIndex, 'Tables array:', props.templateData.tables)"
                ...
                @mouseleave="handleSectionLeave"
                @click="handleSectionClick(`table-section-${tableIndex}`); console.log('PDF Preview: Table section clicked:', tableIndex)"
              >
                <!-- Section Title -->
                <div 
                  ...
                  @mouseleave="handleItemLeave"
                  @click.stop="handleItemClick(`table-section-${tableIndex}`, 'section-title'); console.log('PDF Preview: Section title clicked:', tableIndex)"
                >
                  <div class="flex grow items-start min-w-0">
                    <p class="font-semibold leading-2.25" style="font-size: 7px; color: #6b7280;">{{ table.sectionTitle }}</p>
                  </div>
                </div>
                ...
                <div 
                  ...
                  class="flex gap-1 h-2.25 items-start w-full cursor-pointer transition-all duration-200"
                  :class="getItemHighlightClass(`table-section-${tableIndex}`, 'subsection-title')"
                ...
                >
                  <div class="flex grow items-start min-w-0">
                    <p class="font-semibold leading-2.25 text-black" style="font-size: 7px;">{{ table.subsectionTitle }}</p>
                  </div>
                </div>
                ...
                <div class="flex items-start w-full">
                  ...
                  >
                    ...
                    <div 
                      ...
                      :class="getItemHighlightClass(`table-section-${tableIndex}`, `column-header-${column.id}`)"
                      @mouseenter="handleItemHover(`table-section-${tableIndex}`, `column-header-${column.id}`)"
                      @mouseleave="handleItemLeave"
                      @click.stop="handleItemClick(`table-section-${tableIndex}`, `column-header-${column.id}`)"
                    >
                      <div class="py-0.5 w-full"
                           :class="{
                             'pl-0 pr-2': columnIndex === 0,
                             'pl-2 pr-0': columnIndex === (table.columns?.length || 1) - 1,
                             'px-2': columnIndex > 0 && columnIndex < (table.columns?.length || 1) - 1
                           }">
                        <p class="font-medium leading-2.25" 
                  ...
                  >
                    ...
                    >
                      <div 
                        ...
                        :class="getItemHighlightClass(`table-section-${tableIndex}`, `column-value-${column.id}`)"
                        @mouseenter="handleItemHover(`table-section-${tableIndex}`, `column-value-${column.id}`)"
                        @mouseleave="handleItemLeave"
                        @click.stop="handleItemClick(`table-section-${tableIndex}`, `column-value-${column.id}`)"
                      >
                        <div class="flex items-start overflow-hidden w-full"
                             :class="{
                               'pl-0 pr-2': columnIndex === 0,
                               'pl-2 pr-0': columnIndex === (table.columns?.length || 1) - 1,
                               'px-2': columnIndex > 0 && columnIndex < (table.columns?.length || 1) - 1
                  ...
                  >
                    <div class="flex flex-col items-start justify-center w-full h-3.25">
                      <div class="flex items-center w-full h-full">
                        <div v-if="columnIndex === (table.columns?.length || 0) - 1" 
                          ...
                          <div class="text-right whitespace-nowrap">
                            ...
                            <span class="font-semibold text-black" style="font-size: 7px;">${{ table.total || '0.00' }}</span>
                          </div>
                        </div>
            ...
            <!-- 3️⃣ Summary Section -->
            <div v-if="props.sectionStates.table" class="summary-section flex gap-3 h-4.25 items-end justify-end w-full">
            ...
            >
              ...
              >
                <p v-if="itemSection.sectionTitle" class="item-section-title">{{ itemSection.sectionTitle }}</p>
            ...
            >
              ...
              >
                <p v-if="hInfoSection.sectionTitle" class="h-info-section-title">{{ hInfoSection.sectionTitle }}</p>
                <div class="h-info-columns-container" :style="getColumnContainerStyle(hInfoSection)">
                  <div 
      ...
      <div v-else class="w-full" style="padding: 16px 0;">
        <div ref="exportContainerRef" :style="{ width: '572px', margin: '0 auto' }">
          <!-- Header -->
          <div v-if="props.sectionStates.header"
               :class="getSectionHighlightClass('header-section')"
               @mouseenter="handleSectionHover('header-section')"
               @mouseleave="handleSectionLeave"
               @click="handleSectionClick('header-section')"
               style="margin-bottom: 8px;">
            <h1 style="font-size: 20px; font-weight: 600; margin: 0;">{{ props.templateData.header?.title || 'Title' }}</h1>
            <p v-if="props.templateData.header?.description" style="font-size: 7px; line-height: 9px; margin: 0;">{{ props.templateData.header.description }}</p>
          </div>

          <!-- Info -->
          <div v-if="props.sectionStates.info">
            <div v-for="(infoSection, infoIndex) in (props.templateData.info as any[]) || []" :key="infoSection.id"
                 :class="getSectionHighlightClass('info-section-' + infoIndex)"
                 @mouseenter="handleSectionHover('info-section-' + infoIndex)"
                 @mouseleave="handleSectionLeave"
                 @click="handleSectionClick('info-section-' + infoIndex)"
                 style="margin-bottom: 8px;">
              <p v-if="infoSection.sectionTitle" style="font-size: 7px; font-weight: 600; color: #6b7280; line-height: 9px; margin: 0 0 2px;">{{ infoSection.sectionTitle }}</p>
              <div>
                <div v-for="item in infoSection.items || []"
                     :key="item.id"
                     :class="getItemHighlightClass('info-section-' + infoIndex, item.id)"
                     @mouseenter="handleItemHover('info-section-' + infoIndex, item.id)"
                     @mouseleave="handleItemLeave"
                  ...
                  <p style="font-size: 7px; font-weight: 600; line-height: 9px; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">{{ item.label }}</p>
                  <p style="font-size: 7px; line-height: 9px; margin: 0; word-wrap: break-word;">{{ item.value }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Tables -->
          <div v-if="props.sectionStates.table" :class="getSectionHighlightClass('table-section')" @mouseenter="handleSectionHover('table-section')" @mouseleave="handleSectionLeave" @click="handleSectionClick('table-section')">
            <div v-for="(table, tableIndex) in props.templateData.tables || []" :key="table.id" :class="getSectionHighlightClass('table-section-' + tableIndex)" @mouseenter="handleSectionHover('table-section-' + tableIndex)" @mouseleave="handleSectionLeave" @click="handleSectionClick('table-section-' + tableIndex)" style="margin-bottom: 8px;">
              <p v-if="table.sectionTitle" style="font-size: 7px; color: #6b7280; font-weight: 600; line-height: 9px; margin: 0 0 2px;">{{ table.sectionTitle }}</p>
              <p v-if="table.subsectionTitle" style="font-size: 7px; font-weight: 600; line-height: 9px; margin: 0 0 2px;">{{ table.subsectionTitle }}</p>
              <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">
                <thead>
                  <tr>
                    <th v-for="(column, columnIndex) in table.columns || []"
                        :key="column.id"
                        :class="getItemHighlightClass('table-section-' + tableIndex, 'column-header-' + column.id)"
                        @mouseenter="handleItemHover('table-section-' + tableIndex, 'column-header-' + column.id)"
                        @mouseleave="handleItemLeave"
                        @click.stop="handleItemClick('table-section-' + tableIndex, 'column-header-' + column.id)"
                        :style="{ width: (table.columns?.some(c => c.width) ? (column.width || 0) : (100 / (table.columns?.length || 1))) + '%', padding: (columnIndex === 0 ? '0 8px 0 0' : (columnIndex === (table.columns?.length || 1) - 1 ? '0 0 0 8px' : '0 8px')), 'vertical-align': 'bottom', 'border-bottom': '1px solid #d2d2d2', height: '13px' }">
                      <span :class="{ 'text-right': column.alignment === 'right' }" style="display:block; width:100%; font-size: 7px; color: #919191; line-height: 9px;">{{ column.name || 'Column Name' }}</span>
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(row, rowIndex) in getTableRows(table)" :key="row.id" :class="getItemHighlightClass('table-section-' + tableIndex, 'row-' + rowIndex)" @mouseenter="handleItemHover('table-section-' + tableIndex, 'row-' + rowIndex)" @mouseleave="handleItemLeave" @click.stop="handleItemClick('table-section-' + tableIndex, 'row-' + rowIndex)">
                    <td v-for="(column, columnIndex) in table.columns || []" :key="column.id" :style="{ width: (table.columns?.some(c => c.width) ? (column.width || 0) : (100 / (table.columns?.length || 1))) + '%', padding: (columnIndex === 0 ? '2px 8px 2px 0' : (columnIndex === (table.columns?.length || 1) - 1 ? '2px 0 2px 8px' : '2px 8px')), 'vertical-align': 'top' }">
                      <p class="font-normal text-black" :class="{ 'text-right': column.alignment === 'right' }" style="font-size: 7px; line-height: 9px; margin: 0; word-wrap: break-word;">{{ row.data[column.id] || '' }}</p>
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          <div v-if="props.sectionStates.description">
            <div v-for="(descriptionSection, descIndex) in (props.templateData.description as any[]) || []" :key="descriptionSection.id"
                 :class="getSectionHighlightClass('description-section-' + descIndex)"
                 @mouseenter="handleSectionHover('description-section-' + descIndex)"
                 @mouseleave="handleSectionLeave"
                 @click="handleSectionClick('description-section-' + descIndex)"
              ...
              <p style="font-size: 7px; line-height: 9px; margin: 0; word-wrap: break-word;">{{ descriptionSection.content }}</p>
            </div>
          </div>

          <!-- Item -->
          <div v-if="props.sectionStates.item">
            <div v-for="(itemSection, itemIndex) in (props.templateData.item as any[]) || []" :key="itemSection.id"
                 :class="getSectionHighlightClass('item-section-' + itemIndex)"
                 @mouseenter="handleSectionHover('item-section-' + itemIndex)"
                 @mouseleave="handleSectionLeave"
                 @click="handleSectionClick('item-section-' + itemIndex)"
                 style="margin: 8px 0;">
              <p v-if="itemSection.sectionTitle" style="font-size: 7px; font-weight: 600; color: #6b7280; line-height: 9px; margin: 0 0 2px;">{{ itemSection.sectionTitle }}</p>
              <div>
                <div v-for="item in itemSection.items || []"
                     :key="item.id"
                     :class="getItemHighlightClass('item-section-' + itemIndex, item.id)"
                     @mouseenter="handleItemHover('item-section-' + itemIndex, item.id)"
                     @mouseleave="handleItemLeave"
          ...
          <div v-if="props.sectionStates.hInfo">
            <div v-for="(hInfoSection, hInfoIndex) in (props.templateData.hInfo as any[]) || []" :key="hInfoSection.id"
                 :class="getSectionHighlightClass('h-info-section-' + hInfoIndex)"
                 @mouseenter="handleSectionHover('h-info-section-' + hInfoIndex)"
                 @mouseleave="handleSectionLeave"
                 @click="handleSectionClick('h-info-section-' + hInfoIndex)"
                 style="margin: 8px 0;">
              <p v-if="hInfoSection.sectionTitle" style="font-size: 7px; font-weight: 600; color: #6b7280; line-height: 9px; margin: 0 0 2px;">{{ hInfoSection.sectionTitle }}</p>
              <div style="display: flex; gap: 8px; width: 100%;">
                <div v-for="(column, columnIndex) in hInfoSection.columns || []" :key="column.id"
                     :style="{ 
                  ...
                  <div v-for="item in column.items || []"
                    ...
                    <span style="font-size: 7px; font-weight: 600; line-height: 9px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 80px; flex-shrink: 0;">{{ item.label }}</span>
                    <span style="font-size: 7px; line-height: 9px; word-wrap: break-word; flex: 1;">{{ item.value }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <!-- Footer -->
          <div v-if="props.sectionStates.footer"
               :class="getSectionHighlightClass('footer-section')"
               @mouseenter="handleSectionHover('footer-section')"
               @mouseleave="handleSectionLeave"
               @click="handleSectionClick('footer-section')"
            ...
            <div>
              <p v-if="props.templateData.footer?.info" style="font-size: 7px; line-height: 9px; margin: 0;">{{ props.templateData.footer.info }}</p>
              <p v-if="props.templateData.footer?.name" style="font-size: 7px; line-height: 9px; margin: 0; text-align: center;">{{ props.templateData.footer.name }}</p>
              <p style="font-size: 7px; line-height: 9px; margin: 0; text-align: right;">Page: 1 of 1</p>
            </div>
          </div>
        </div>
      </div>
...
const debugTableData = computed(() => {
  if (props.sectionStates.table && props.templateData.tables) {
    console.log('PdfPreviewPanel: table data updated', props.templateData.tables.map(table => ({
      id: table.id,
